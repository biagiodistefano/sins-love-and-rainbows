...
def test_short_url(api_client: Client) -> None:
    response = api_client.post(
        "/api/s/create", json.dumps({"url": "https://www.biagiodistefano.io"}),
    ...
    response = api_client.put(
        "/api/s/bd", json.dumps({"url": "https://www.biagiodistefano.io", "short_url": "bd2"}),
        content_type="application/json"
        )
    assert response.status_code == 200
    ...
    response = api_client.put(
        "/api/s/bd2", json.dumps({"url": "https://www.biagiodistefano.io/bd2"}),
        content_type="application/json"
        )
    assert response.status_code == 200
    assert response.json()["short_url"] == "bd2"
